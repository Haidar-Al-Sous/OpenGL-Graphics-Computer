...
glBegin(GL_TRIANGLE_STRIP);
	...
	for (float aa = 0.0f; aa <= (2.0f*3.14f); aa += 0.1f)
	{
	float x =  cos(aa)*cos(a);
	float y =  sin(aa);
	float z = cos(aa)*sin(a);
			glColor3f(1.0f, 1.0f, 1.0f);
	if(a>3.14)
		glColor3f(1,0,0);
	glVertex3f(x, y, z);
	glVertex3f(cos(aa)*cos(a+0.1), y, cos(aa)*sin(a+0.1));
...
void a()
{
	...
	for (float a = 0.0f; a <= (2.0f*3.14f); a += 0.1f)
		{float z =  cos(a)*1.5;
		 ...
		 glVertex3f(x,0,z);
		 glVertex3f(x,3,z);
		 glVertex3f(sin(a+0.1)*1.5,3,cos(a+0.1)*1.5);
    	 glVertex3f(sin(a+0.1)*1.5,0,cos(a+0.1)*1.5);
	}
	glEnd();
	}}
GLfloat hh,k,xxx,yy,f;
